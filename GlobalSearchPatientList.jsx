...
const GlobalSearchPatientList = ({ onClose }) => {
    ...
    const clientIdList =
        selectedBranchIds.length > 0 ? selectedBranchIds.join(',') : '1';
    ...
    const getPatientList = async (
        ...
        try {
            ...
            const finalClientIdList =
                branchIds?.length > 0 ? branchIds.join(',') : '1';

            const response = await api.get('Patient/search-patient-topCount', {
                params: {
                    clientIdList: finalClientIdList,
                    searchText,
    ...
    useEffect(() => {
        if (selectedBranchIds.length > 0) {
            getPatientList('', '100', selectedBranchIds);
        }
        ...
        <View style={[themed.childScreen2, tw`flex-1`]}>
            ...
            <TouchableOpacity
                ...
                style={[
                    themed.childScreen2,
                    themed.border,
                    tw`mx-4 mt-3 p-3 rounded-xl flex-row items-center justify-between`,
                ]}>
                <View style={tw`flex-1`}>
                    <Text style={[themed.inputText, tw`font-bold`]}>Selected Branches</Text>
