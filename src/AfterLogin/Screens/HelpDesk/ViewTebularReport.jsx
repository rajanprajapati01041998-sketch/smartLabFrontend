...
const ViewLabReport = () => {
    ...
    // console.log("FULL PARAMS:", route.params?.item?.PatientInvestigationId);
    const PatinetInvestingationId = route.params?.item?.PatientInvestigationId
    const getReport = async () => {
        try {
            setLoading(true)
            setError(false)
            const response = await api.get(`DeltaReport/download-delta-report?PatientInvestigationIdList=${PatinetInvestingationId}&isHeaderPNG=0&PrintBy=${userId}&branchId=${loginBranchId}&ViewReport=true`)
