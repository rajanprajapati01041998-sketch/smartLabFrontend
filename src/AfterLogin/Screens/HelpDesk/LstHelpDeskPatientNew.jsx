...
const ListHelpDeskPatientNew = () => {
  ...
  // Header options state - keeping these for view functionality
  ...
  const [actionType, setActionType] = useState(null); // 'download' or 'view'
  ...
  // Function to show modal for download
  const showDownloadModal = (item) => {
    setSelectedItemForAction(item);
    setActionType('download');
    setModalVisible(true);
  };

  // Function to show modal for view
  ...
  // Direct download functions for each option
  const handleDownloadWithoutLetterhead = () => {
    if (!selectedItemForAction) return;
    const { PatientInvestigationId, PatientName, ReportTypeId } = selectedItemForAction;
    setModalVisible(false);
    handleDownloadReport(PatientInvestigationId, PatientName, ReportTypeId, false, false, false);
    setSelectedItemForAction(null);
    setActionType(null);
  };

  const handleDownloadWithLetterhead = () => {
    if (!selectedItemForAction) return;
    const { PatientInvestigationId, PatientName, ReportTypeId } = selectedItemForAction;
    setModalVisible(false);
    handleDownloadReport(PatientInvestigationId, PatientName, ReportTypeId, true, false, true);
    setSelectedItemForAction(null);
    setActionType(null);
  };

  const handleDownloadWithGlobalLetterhead = () => {
    if (!selectedItemForAction) return;
    const { PatientInvestigationId, PatientName, ReportTypeId } = selectedItemForAction;
    setModalVisible(false);
    handleDownloadReport(PatientInvestigationId, PatientName, ReportTypeId, true, false, true);
  ...
  // View functions for each option
  const handleViewWithoutLetterhead = () => {
    if (!selectedItemForAction) return;
    const item = selectedItemForAction;
    setModalVisible(false);
    navigateToView(item, false, false, false);
    setSelectedItemForAction(null);
    setActionType(null);
  };

  const handleViewWithLetterhead = () => {
    if (!selectedItemForAction) return;
    const item = selectedItemForAction;
    setModalVisible(false);
    navigateToView(item, true, true, false);
    setSelectedItemForAction(null);
    setActionType(null);
  };

  const handleViewWithGlobalLetterhead = () => {
    if (!selectedItemForAction) return;
    const item = selectedItemForAction;
    setModalVisible(false);
    navigateToView(item, true, false, true);
    setSelectedItemForAction(null);
    setActionType(null);
  };

  const navigateToView = (item, isPrintHeader, loginHeader, mainHeader) => {
    ...
    if (reporTypeId === 2) {
      try {
        ...
        await config({
          ...
          'GET',
          `${API_BASE_URL}ReportPrint/DownloadCombinedReport?ptInvstId=${id}&isHeaderPNG=${isPrintHeader ? 1 : 0}&printBy=${userId}&branchId=${selectedBranchId}`
        );
      ...
      handleDownloadTebularReport(id, name, reporTypeId, isPrintHeader, loginHeader, mainHeader);
    }
  };

  const handleDownloadTebularReport = async (id, name, reporTypeId, isPrintHeader, loginHeader, mainHeader) => {
    try {
      setDownloadingId(id);
      // console.log("report Id headre =", isPrintHeader);
      // console.log("report Id login =", loginHeader);
      // console.log("report Id main =", mainHeader);
      // return;
      const { config, fs } = RNFetchBlob;
      ...
      // console.log("report dpwnload:", isHeaderPNG)
      // return;


      const url =
        `${API_BASE_URL}DeltaReport/download-delta-report?PatientInvestigationIdList=${id}&isHeaderPNG=${isHeaderPNG ? 1 : 0}&PrintBy=${userId}&branchId=${selectedBranchId}&ViewReport=false`;
            ...
            <View style={tw`flex-row items-center flex-1`}>
              ...
              <View style={tw`flex-1 justify-start items-start`}>
                ...
                <View style={tw`flex-row items-center gap-2`}>
                  <Text style={[tw`mt-0.5 text-xs text-gray-900 font-semibold`]}>
                    {item.UHID || 'N/A'}
                  </Text>
          ...
          <View style={[themed.transactionDivider, themed.globalCard, tw`p-4 border-t`]}>
            <View style={tw`gap-2`}>
              ...
              <View style={tw`flex-row flex-wrap gap-3 mb-2`}>
                {item.CurrentAge && <View style={tw`flex-row items-center`}>
                  <MaterialCommunityIcons name="calendar" size={14} color={themed.iconMuted} />
                  <Text style={[tw`ml-1 text-xs text-gray-900`]}>
              ...
              {item.BillDate && (
                <View style={tw`flex-row items-center justify-between`}>
                  <View style={tw`flex-row items-center`}>
                    ...
                    <Text style={[themed.labeltext, tw`ml-2 text-xs`]}>Bill Date</Text>
                  </View>
                  <Text style={[themed.labeltext, tw`text-xs`]}>{formatBillDateTime(item.CreatedOn)}</Text>
                </View>
              )}
              ...
              {item.SampleCollectedOn && (
                <View style={tw`flex-row items-center justify-between`}>
                  <View style={tw`flex-row items-center`}>
                    ...
                    <Text style={[themed.labeltext, tw`ml-2 text-xs`]}>Sample Collected</Text>
                  </View>
                  <Text style={[themed.labeltext, tw`text-xs`]}>{item.SampleCollectedOn}</Text>
                </View>
              )}
              ...
              {item.ResultDoneOn && (
                <View style={tw`flex-row items-center justify-between`}>
                  <View style={tw`flex-row items-center`}>
                    ...
                    <Text style={[themed.labeltext, tw`ml-2 text-xs`]}>Result Done</Text>
                  </View>
                  <Text style={[themed.labeltext, tw`text-xs`]}>{item.ResultDoneOn}</Text>
                </View>
              )}
              ...
              {item.ReportApprovedOn && (
                <View style={tw`flex-row items-center justify-between`}>
                  <View style={tw`flex-row items-center`}>
                    ...
                    <Text style={[themed.labeltext, tw`ml-2 text-xs`]}>Report Approved</Text>
                  </View>
                  <Text style={[themed.labeltext, tw`text-xs`]}>{item.ReportApprovedOn}</Text>
                </View>
              )}
              ...
              {item.DispatchedOn && (
                <View style={tw`flex-row items-center justify-between`}>
                  <View style={tw`flex-row items-center`}>
                    ...
                    <Text style={[themed.labeltext, tw`ml-2 text-xs`]}>Dispatched</Text>
                  </View>
                  <Text style={[themed.labeltext, tw`text-xs`]}>{formatDate(item.DispatchedOn)}</Text>
                </View>
              )}
            </View>
            ...
            {walletData.balanceMain >= 0 && item?.IsReportApproved === 1 && (
              <View style={tw`flex-row gap-2 mt-4`}>
                <TouchableOpacity
                  onPress={() => showDownloadModal(item)}
    ...
    <View style={[themed.screen, tw`flex-1`]}>
      <View style={[themed.globalCard, tw`px-4 py-3 border-b rounded-none`]}>
        ...
        <View style={tw`flex-row items-center gap-3`}>
          <View style={[themed.inputBox, tw`flex-1 flex-row items-center px-3 h-[50px] rounded-xl`]}>
      ...
      {/* Common Modal for Download and View Options */}
      <Modal
      ...
      >
        ...
        >
          ...
          <View style={tw`flex-1 bg-black/80 justify-center items-center`}>
            <View style={[themed.childScreen2, themed.border, tw`w-[99%] rounded-2xl p-4`]}>
              <View style={tw`flex-row justify-between items-center mb-4`}>
                <Text style={[themed.listItemText, tw`text-lg font-bold`]}>
                  {actionType === 'download' ? 'Download Options' : 'View Options'}
                </Text>
              ...
              <View style={tw`gap-4 flex-row`}>
                {/* Without Letterhead Button */}
                <TouchableOpacity
                  onPress={actionType === 'download' ? handleDownloadWithoutLetterhead : handleViewWithoutLetterhead}
                  style={[tw`p-2 rounded-xl w-[29.5%] bg-blue-600 `]}
                  activeOpacity={0.7}
                >
                  <Text style={[tw`text-center font-semibold text-white pt-2`]}>
                    Without Letterhead
                  </Text>
                </TouchableOpacity>

                {/* Letterhead (Login Header) Button */}
                <TouchableOpacity
                  onPress={actionType === 'download' ? handleDownloadWithLetterhead : handleViewWithLetterhead}
                  style={[themed.border, tw`py-4 rounded-xl w-[29.5%] bg-blue-600`]}
                  activeOpacity={0.7}
                >
                ...
                {/* Global Letterhead (Main Header) Button */}
                <TouchableOpacity
                  onPress={actionType === 'download' ? handleDownloadWithGlobalLetterhead : handleViewWithGlobalLetterhead}
                  style={[
                    tw`py-4 rounded-xl border-2 w-[29.5%] bg-purple-900 border-purple-700`,
                  ]}
                  activeOpacity={0.7}
                >
                  <Text style={[
                    tw`text-center font-semibold text-md text-white`,
                  ]}>
                    Letterhead (Main)
                  </Text>
                </TouchableOpacity>
...
export default ListHelpDeskPatientNew;